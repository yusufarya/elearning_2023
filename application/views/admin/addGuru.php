...
$SQRY = "SELECT MAX(nomor_identitas) AS identitas FROM users WHERE role_id = '2' ";
$getLastNo = $this->db->query($SQRY)->row_array();
if ($getLastNo['identitas'] == '') {
    $nomor = 'IDT' . date('ymd') . '0001';
} else {
    $nomor = $getLastNo['identitas'];
    $nomor = substr($nomor, -4) + 1;
    $nomor = 'IDT' . date('ymd') . sprintf('%04d', $nomor);
}
                ...
                <form action="<?= base_url('ControllerUser/insertDataUser') ?>" method="POST" enctype="multipart/form-data">
                    <div class="row m-2">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <a href="<?= base_url('listTeacher') ?>" class="btn btn-dark  mx-3" style="float:right">Batal</a>
                        </div>
                    </div>
                </form>
            </div>
